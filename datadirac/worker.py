...
class MPINode:
    ...
    def get_data(self):
        ...
        sd.load_dataframe( op.join( self.working_dir, self.data_file) )
    ...
    def _get_data_files(self):
        ...
        """
        while not op.exists(op.join( self.working_dir, self.meta_file)):
            try:
                ...
                k.get_contents_to_filename(
                        op.join( self.working_dir, self.meta_file))
            except:
                time.sleep(random.random())

        while not op.exists(op.join( self.working_dir, self.data_file)):
            conn = boto.connect_s3()
            try:
                b = conn.get_bucket(self.ds_bucket)
                k = Key(b)
